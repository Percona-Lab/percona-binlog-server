...
public:
  ...
  // When `ssl_capability_enabled` is true, the server greeting generated by
  // this context advertises CLIENT_SSL so a TLS-capable client can respond
  // with an SSLRequest. The value is fixed at construction — a
  // connection_context is created per session, and whether the listener has
  // an SSL acceptor context is a per-listener property known before the
  // session starts.
  connection_context(std::string_view server_username,
                     ...
                     bool ssl_capability_enabled = false);
  ...
  // Flipped by the network layer once the underlying transport has been
  // upgraded to TLS. Enables the caching_sha2_password cleartext-after-0x04
  // fast path in begin_authentication(), matching Percona Server behaviour.
  void mark_transport_secure() noexcept;

  // True iff the last parsed client greeting has CLIENT_SSL set in its own
  // capability flags — irrespective of whether the server advertised SSL.
  // Callers combine this with knowledge of the listener's SSL configuration
  // to detect a "client wants SSL against a plaintext-only server" case and
  // handle it explicitly, matching Percona Server's behaviour.
  // Not noexcept because std::bitset<>::test() is not noexcept.
  [[nodiscard]] bool client_requested_ssl() const;

  // True iff the last parsed client greeting has the SSLRequest shape:
  // shared caps include CLIENT_SSL and the username is empty (the classic
  // protocol codec truncates the packet after the 23-byte filler in that
  // case, leaving all subsequent fields empty).
  // Not noexcept because std::bitset<>::test() is not noexcept.
  [[nodiscard]] bool is_sslrequest_greeting() const;
...
private:
  ...
  bool transport_is_secure_{false};
  bool ssl_capability_enabled_;
