...
public:
  ...
  explicit flag_set(element_type element) noexcept
      : bits_{static_cast<underlying_type>(element)} {
  ...
  this_type &operator&=(element_type other) noexcept {
    return *this &= this_type{other};
  }
...
template <typename E>
  requires std::is_enum_v<E> && std::is_unsigned_v<std::underlying_type_t<E>>
[[nodiscard]] flag_set<E> operator|(flag_set<E> first, E second) noexcept {
  auto res{first};
  res |= second;
  return res;
}
template <typename E>
  requires std::is_enum_v<E> && std::is_unsigned_v<std::underlying_type_t<E>>
[[nodiscard]] flag_set<E> operator|(E first, flag_set<E> second) noexcept {
  flag_set<E> res{first};
  res |= second;
  return res;
}
