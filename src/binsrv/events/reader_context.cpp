...
void reader_context::start_transaction(const event &current_event) {
  ...
  case code_type::anonymous_gtid_log: {
    ...
    if (!current_event.get_post_header<binsrv::events::code_type::gtid_log>()
             .get_gtid()
             .is_empty()) {
      util::exception_location().raise<std::logic_error>(
    ...
    const auto expected_transaction_length_raw{
        current_event.get_body<binsrv::events::code_type::anonymous_gtid_log>()
  ...
  case code_type::gtid_log: {
    transaction_gtid_ =
        current_event.get_post_header<binsrv::events::code_type::gtid_log>()
            .get_gtid();
    ...
    const auto expected_transaction_length_raw{
        current_event.get_body<binsrv::events::code_type::gtid_log>()
            .get_transaction_length_raw()};
    if (!std::in_range<std::uint32_t>(expected_transaction_length_raw)) {
      util::exception_location().raise<std::logic_error>(
  ...
  case code_type::gtid_tagged_log: {
    const auto &gtid_tagged_log_body{
        current_event.get_body<binsrv::events::code_type::gtid_tagged_log>()};
