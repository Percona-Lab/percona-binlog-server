...
// Generates a fresh 2048-bit RSA key pair on the fly, writes the SubjectPublic
// KeyInfo-format public key to 'pub_path' and the PKCS#8-format private key
// (unencrypted) to 'priv_path'. The authenticator no longer embeds default
// keys, so every test that needs one bring its own via this helper.
void write_temp_rsa_key_pair(const std::string &pub_path,
...
constexpr std::string_view test_rsa_pub_path{
    "/tmp/pbs_test_server_rsa_public.pem"};
constexpr std::string_view test_rsa_priv_path{
    "/tmp/pbs_test_server_rsa_private.pem"};
...
BOOST_AUTO_TEST_CASE(EmptyServerRsaPathsAreAcceptedAtConstruction) {
  // Empty paths on both sides are a valid ctor input: the authenticator
  // simply builds without RSA key material and any subsequent full-auth
  // attempt fails then. The "operator must configure the pair for 'pull'"
  // invariant is enforced by binsrv::pbs_listener_config::validate() and
  // operations/pull_operation, not by the authenticator itself.
  BOOST_CHECK_NO_THROW(
      minimysql::caching_sha2_password_authenticator("password"));
}

BOOST_AUTO_TEST_CASE(MatchingGreetingScrambleStillDrivesFullAuth) {
  // PBS has no SHA-2 digest cache, so begin_authentication always
  // enqueues 0x04 (perform_full_authentication) and drives every login
  // through the RSA / PEM handshake, even when the client's greeting
  // scramble already matches the configured password. Once a digest cache
  // is added, this case will flip back to 0x03 (fast auth success); the
  // scramble() / verify_greeting_scramble() helpers are kept alive in the
  // authenticator for that day.
  ...
  write_temp_rsa_key_pair(std::string{test_rsa_pub_path},
                          std::string{test_rsa_priv_path});
  minimysql::caching_sha2_password_authenticator authenticator{
      password, test_rsa_pub_path, test_rsa_priv_path};
...
BOOST_AUTO_TEST_CASE(FullAuthRsaPathSucceedsViaPublicKeyRequest) {
  ...
  // The greeting scramble is what a real client sends in the handshake
  // response. Under always-full-auth its value is not consulted here (the
  // authenticator will drive full auth regardless), but we pass a properly
  // computed scramble to mirror what a real mysql client would send.
  const auto greeting_scramble{
      minimysql::caching_sha2_password_authenticator::scramble(password, salt)};
  authenticator.begin_authentication(username, username, greeting_scramble,
                                     salt, false, encoder);
