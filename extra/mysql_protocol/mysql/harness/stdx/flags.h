...
#ifndef MYSQL_HARNESS_STDX_FLAGS_H
#define MYSQL_HARNESS_STDX_FLAGS_H
...
template <class E> inline constexpr bool is_flags_v = is_flags<E>::value;
...
public:
  ...
  constexpr flags &operator=(const flags &other) = default; //!< copy assignment
  ...
  constexpr flags(enum_type value) : v_{static_cast<impl_type>(value)} {}
   ...
   * flags & flags -> flags
   */
  friend constexpr flags operator&(flags left, flags right) noexcept {
    return flags{static_cast<impl_type>(left.v_ & right.v_)};
  }
   ...
   *
   * @param value underlying value to set.
   */
  constexpr void underlying_value(underlying_type value) noexcept {
    v_ = value;
  }
   ...
   */
  [[nodiscard]] constexpr underlying_type underlying_value() const noexcept {
    return v_;
  }
  ...
  [[nodiscard]] constexpr size_type size() const noexcept {
    return CHAR_BIT * sizeof(underlying_type);
  }
...
private:
  constexpr explicit flags(impl_type value) noexcept : v_{value} {}
 ...
 */
template <class E>
  requires stdx::is_flags_v<E>
constexpr stdx::flags<E> operator|(E left, E right) noexcept {
  return stdx::flags<E>(left) | right;
}
...
#endif // MYSQL_HARNESS_STDX_FLAGS_H
