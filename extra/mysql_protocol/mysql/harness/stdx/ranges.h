 ...
 */
template <class V> class enumerate_view {
private:
  using Base = V;
...
public:
  using value_type = std::iter_value_t<iterator<true>>;

  constexpr enumerate_view() = default;

  constexpr explicit enumerate_view(V base) : base_(std::forward<Base>(base)) {}
...
template <class V> template <bool Const> class enumerate_view<V>::iterator {
private:
...
public:
  ...
  using reference =
      std::tuple<index_type, std::ranges::range_reference_t<Base>>;
  using value_type = std::tuple<index_type, std::ranges::range_value_t<Base>>;
...
#endif // MYSQL_HARNESS_STDX_RANGES_ENUMERATE_H
